...
inoremap <silent> <C-]> <C-r>=TriggerSnippetAbbr("\<C-]>", 1)<CR>
...
function! TriggerSnippetAbbr(char, start)
    let result = TriggerSnippet(a:char, a:start)
    if result != "\<C-]>"
	return result
    endif

    call feedkeys("\<Space>\<Backspace>")
    return ''
endfunction
